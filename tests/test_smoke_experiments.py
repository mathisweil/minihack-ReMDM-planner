...
def test_run_ablations_exposes_the_gdelta_measurement():
    """The gradient measurement is a mode of the suite, not its own script,
    so its results land in a run directory and get published with the rest.
    """
    result = run_cli(RUN_ABLATIONS, "--help")
...
def test_measure_gdelta_errors_cleanly_without_a_checkpoint(tmp_path):
    """No checkpoint resolves -> a usage error, not a traceback."""
    result = run_cli(
        RUN_ABLATIONS,
        "--measure-gdelta",
        "--output-dir",
    ...
    assert "--checkpoint is required for --measure-gdelta" in combined
